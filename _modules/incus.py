...
# ========== Certificate Trust Management Functions ==========

def certificate_list(recursion=1):
    """
    List trusted certificates from Incus trust store.

    CLI Example:

    .. code-block:: bash

        salt '*' incus.certificate_list
        salt '*' incus.certificate_list recursion=0

    :param recursion: Recursion level (0=URLs, 1=full records)
    :return: Dictionary with 'success' and 'certificates'
    """
    client = _client()
...
def certificate_add(certificate_path, name=None, cert_type="client"):
    """
    Add a certificate into Incus trust store.

    CLI Example:

    .. code-block:: bash

        salt '*' incus.certificate_add /etc/salt/pki/incus/client.crt name=salt-cloud

    :param certificate_path: Path to PEM certificate file
    :param name: Optional trust entry name
    :param cert_type: Certificate type (default: client)
    :return: Dictionary with operation result
    """
    if not certificate_path:
        return {"success": False, "error": "certificate_path is required"}

    if not os.path.exists(certificate_path):
        return {
            "success": False,
            "error": f"Certificate file does not exist: {certificate_path}",
        }

    try:
        with open(certificate_path, "r", encoding="utf-8") as fp:
            certificate_contents = fp.read()
    except OSError as exc:
        return {"success": False, "error": f"Failed to read certificate file: {exc}"}

    if not certificate_contents.strip():
        return {
            "success": False,
            "error": f"Certificate file is empty: {certificate_path}",
        }
    ...
    if result.get("error_code") != 0:
        return {
            "success": False,
            "error": result.get("error", "Failed to add trusted certificate"),
        }

    return {"success": True, "message": "Certificate added to trust store"}


def certificate_remove(fingerprint):
    """
    Remove a certificate from Incus trust store by fingerprint.

    CLI Example:

    .. code-block:: bash

        salt '*' incus.certificate_remove <fingerprint>
